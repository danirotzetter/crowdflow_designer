<?php
/*
* https://github.com/jackbot/PHP-Mechanical-Turk/blob/master/MechanicalTurk.class.php April 2013
...
class MechanicalTurk {
	...
	public function __construct($access = null, $secret = null) {
		...
		if ($access === null || $secret === null) { die('Please provide your AWS Access Key and Secret Key'); }
	...
	*/	
	public function createHit($question, $params = null) {
		...
		$url .= '&Signature=' . $this->generateSignature(Yii::app()->controller->module->serviceName, 'CreateHIT', $ts);
		$url .= '&Timestamp=' . $ts;
	...
	*/	
	public function getReviewableHITs($page_size = 50) {
		...
		$url .= '&Signature=' . $this->generateSignature(Yii::app()->controller->module->serviceName, 'GetReviewableHITs', $ts);
		$url .= '&Timestamp=' . $ts;
	...
	*/	
	public function getHITResult($hid, $page_size = 50) {
		...
		$url .= '&Signature=' . $this->generateSignature(Yii::app()->controller->module->serviceName, 'GetAssignmentsForHIT', $ts);
		$url .= '&Timestamp=' . $ts;
		$url .= '&HITId=' . $hid;
		$url .= '&PageSize=' . $page_size;
	...
	*/	
	public function getHITDetails($hid) {
		...
		$url .= '&Signature=' . $this->generateSignature(Yii::app()->controller->module->serviceName, 'GetHIT', $ts);
		$url .= '&Timestamp=' . $ts;
		$url .= '&HITId=' . $hid;
	...
	*/	
	public function approveHIT($hit_id) {
		...
		$url .= '&Signature=' . $this->generateSignature(Yii::app()->controller->module->serviceName, 'ApproveAssignment', $ts);
		$url .= '&Timestamp=' . $ts;
	...
	*/	
	public function removeHIT($hid) {
		...
		$url .= '&Signature=' . $this->generateSignature(Yii::app()->controller->module->serviceName, 'DisposeHIT', $ts);
		$url .= '&Timestamp=' . $ts;
		$url .= '&HITId=' . $hid;
	...
	*/	
	public function getAccountBalance($nice = true) {
		...
		$url .= '&Signature=' . $this->generateSignature(Yii::app()->controller->module->serviceName, 'GetAccountBalance', $ts);
		$url .= '&Timestamp=' . $ts;
	...
	*/
	private function startUrl() {
		return Yii::app()->controller->module->baseUrl . 'AWSAccessKeyId=' . $this->aws_access_key;
	}
